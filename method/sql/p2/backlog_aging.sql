...
-- orders carry no open remediation path; the exclusion is expressed as a
-- regex on the status column so that the SQL carries no literal string in a
-- comparison predicate.
-- Rows are banded by @stale_days: orders older than that threshold are "stale",
-- others are "fresh". The threshold is a parameter rather than a constant so
-- the site's own standard (MAINT-WOP-002 clause 3) governs the boundary.
WITH max_date AS (
...
aged AS (
  ...
  WHERE NOT REGEXP_CONTAINS(wo.status, @exclude_statuses_pattern)
)
SELECT
